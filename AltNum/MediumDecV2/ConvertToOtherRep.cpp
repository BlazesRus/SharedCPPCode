...
inline void BlazesRusCode::MediumDecV2Base::ConvertEByDivToNumByDiv()
{
	BasicIntDivOp(-ExtraRep);
	ExtraRep = 0;
	__int64 SRep;
...
inline void BlazesRusCode::MediumDecV2Base::ConvertFromEFractionalToNorm()
{
	int divisor = DecimalHalf;
	DecimalHalf = 0;
	ExtraRep = 0;
	...
	SRep *= IntValue;
	__int64 divRes = SRep / MediumDecV2Base::DecimalOverflowX;
	DecimalHalf = (int)(SRep - MediumDecV2Base::DecimalOverflowX * divRes);
	if (divRes == 0 && IsNegative)
	{
		if (DecimalHalf == 0)
			IntValue = 0;
		else
			IntValue = MediumDecV2Base::NegativeRep;
	}
	else if (IsNegative)
		IntValue = (int)-divRes;
	else
...
inline void BlazesRusCode::MediumDecV2Base::ConvertToNormType(const RepType& repType)
{
	switch (repType)
	{
	...
	{
		MediumDecV2Base Res = IntValue < 0 ? MediumDecV2Base(DecimalHalf, 0) : MediumDecV2Base(DecimalHalf, 0);
		Res /= ExtraRep;
...
inline const MediumDecV2Base BlazesRusCode::MediumDecV2Base::ConvertAsNormType(const RepType& repType)
{
